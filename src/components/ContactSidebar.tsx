...
function IconQuote({ className = 'h-6 w-6' }: { className?: string }) {
  return (
...
function IconWeChat({ className = 'h-6 w-6' }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor" aria-hidden>
...
function QrCard({ label, icon, src, alt }: { label: string; icon: ReactNode; src: string; alt: string }) {
  return (
...
export function ContactSidebar() {
  ...
  const panelCol = open ? `${PANEL_W}px` : '0px'
  ...
  return (
    ...
    >
      <div
        className="overflow-hidden rounded-l-xl border border-r-0 border-white/10 bg-graphite-light shadow-2xl transition-[grid-template-columns] duration-200 ease-out"
        style={{
          display: 'grid',
          gridTemplateColumns: `${panelCol} ${STRIP_W}px`,
          gridTemplateRows: 'auto 68px 68px auto auto',
        }}
      >
        {/* Panel header */}
        <div className="overflow-hidden border-b border-white/10 px-4 py-2.5" style={{ minWidth: PANEL_W }}>
          <p className="text-[10px] font-semibold uppercase tracking-widest text-slate-muted">Contact</p>
        </div>
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
          className="flex flex-col items-center justify-center gap-0.5 border-b border-l border-white/10 bg-accent py-2 text-[9px] font-bold uppercase leading-tight tracking-wider text-graphite"
          aria-expanded={open}
        >
        ...
        <a
          href="#upload"
          className="flex items-center overflow-hidden border-b border-white/10 px-4 transition hover:bg-white/[0.04]"
          style={{ minWidth: PANEL_W }}
        >
          <div>
            <p className="text-sm font-bold uppercase tracking-wide text-accent">Get a quote</p>
            <p className="mt-0.5 text-xs text-slate-muted">Upload your bill · 10% price in 1 day</p>
          </div>
        </a>
        <a
          href="#upload"
          title="Get a quote"
          className="flex items-center justify-center border-b border-l border-white/10 bg-graphite text-accent transition hover:bg-white/5"
        >
          <IconQuote />
        </a>

        {/* WhatsApp number row — same height as icon */}
        <a
        ...
        >
          <div>
            ...
            <p className="mt-1 text-lg font-semibold leading-tight text-white">{WHATSAPP_DISPLAY}</p>
          </div>
        </a>
        <a
          href={WHATSAPP_CHAT_URL}
        ...
        {/* WhatsApp QR — panel only */}
        <div className="overflow-hidden border-b border-white/10 px-3 py-3" style={{ minWidth: PANEL_W }}>
          <QrCard
            label="Scan · WhatsApp"
        ...
        <div className="border-b border-l border-white/10 bg-graphite" aria-hidden />

        {/* WeChat QR + icon on same row track */}
        <div className="overflow-hidden px-3 py-3" style={{ minWidth: PANEL_W }}>
          <QrCard
            label="Scan · WeChat"
        ...
        <div className="flex items-center justify-center border-l border-white/10 bg-graphite py-4 text-accent">
          <IconWeChat />
        </div>
      </div>
    </div>
    ...
    <>
      ...
      <div className="fixed bottom-[4.25rem] right-0 left-0 z-50 md:hidden">
        ...
        >
          ...
          Contact · Quote · WhatsApp · WeChat
        </button>
        {open && (
          <div className="mx-3 mt-2 max-h-[70vh] overflow-y-auto rounded-xl border border-white/10 bg-graphite-light p-3 shadow-xl">
            <a href="#upload" onClick={() => setOpen(false)} className="block py-2 font-semibold text-accent">
              Get a quote
            </a>
            <a
              href={WHATSAPP_CHAT_URL}
